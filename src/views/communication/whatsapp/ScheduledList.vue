<template>
  <div class="col-md-12 border mt-4">
    <div class="d-flex flex-column flex-sm-row justify-content-sm-between">
      <div>
        <div class="text-head font-weight-bold h2 py-0 my-0 text-black">Schedules</div>
        <div class="s-18">Showing all Schedules Data</div>
      </div>
    </div>
    <el-tooltip
      class="box-item"
    ...
    <Table
      :data="scheduledWhatsappList"
      :headers="scheduledListHeaders"
    ...
    >
      ...
      <template #chatRecipients="{ item }">
        <span class="small-text">
          <el-tag
            class="ml-2"
            type="info"
            v-for="(recipient, index) in item.chatRecipients"
            :key="index"
            >{{ recipient }}</el-tag
          >
        </span>
      ...
      <template #whatsappAttachment="{ item }">
        <span class="small-text">
          ...
          >&nbsp;
          <sub class="text-grey">{{
            item.base64File ? formatBytes(item.whatsappAttachment.fileSize) : "-"
          }}</sub>
        </span>
      </template>
      <template #action="{ item }">
        <el-dropdown trigger="click">
          ...
          <template #dropdown>
            <el-dropdown-menu>
              <el-dropdown-item> View file </el-dropdown-item>
              <el-dropdown-item>
                <div
                  @click.prevent="showConfirmModal(item.id, 'single')"
                  class="text-color"
                >
                  Delete
                </div>
    ...
    <div class="row" v-if="scheduledWhatsappList.length <= 0">
...
export default {
  ...
  setup() {
    const scheduledWhatsappList = ref(
      store.getters["communication/scheduledWhatsappList"]
    );
    ...
    const deleteWhatsappSchedule = async (id, type) => {
      let payload =
        type == "single" ? id : markedSchedules.value.map((i) => i.id).join(",");
      try {
        let { data } = await axios.delete(
          `/api/Messaging/DeleteWhatsAppScheduledMessages?scheduledMessageIdList=${payload}`
        );
