  ...
  <div class="container-top" :class="{ 'container-wide': lgAndUp || xlAndUp }">
      ...
      <div class="row">
        ...
        <div class="col-md-11 ml-2 text-center" v-if="gatewayNotification">
          <div class="text-danger font-weight-bold small">
            Please select one or two payment gateway
          </div>
        </div>
      </div>
    ...
    <el-dialog v-model="displayResponsive" title="" :width="mdAndUp || lgAndUp || xlAndUp ? `50%` : `90%`">
      <div class="row">
        <div class="col-md-12">
          <p>
            You have no income account to create a offering item, go to Chart of Account
            and click 'Update Account' to update your accounts.
          </p>
        </div>
      </div>
      <template #footer>
  ...
  setup(prop, { emit }) {
          ...
          .catch((err) => {
            ...
            if (err.response) {
              ...
              ElMessage({
                type: "warning",
                message:
                  "Request Delayed,Make sure you have a strong internet connection",
                duration: 3000,
              });
            } else {
              ElMessage({
    ...
    const gateways = computed(() => {
      ...
      const x = paymentGateWaysDb.value.filter((i) => {
        ...
        return (
          i.name.toLowerCase().includes("paystack") ||
          i.name.toLowerCase().includes("flutterwave")
        );
      });
